...
输出示例：15:32:41 up 45 days, 3:15, 1 user, load average: 0.00, 0.01, 0.05

* up 45 days 表示系统已连续运行 45 天

* load average 后面的三个数字分别表示过去 1分钟、5分钟、15分钟的系统平均负载
...
这会列出每个 CPU 核心的详细信息。通常与 grep 结合使用，例如 `grep "processor" /proc/cpuinfo | wc -l` 来统计逻辑CPU核心总数。

#### 2. 内存信息
* 查看物理内存和交换空间的使用情况（最常用）：
```bash
free -h
```
...
3. 然后重启（`reboot`），就可以使用root用户登录操作系统了。
...
```
mongodump -h 127.0.0.1 --port 27017 --authenticationDatabase admin -d Print -u='root' -p='Numb8520' -o ./backup/
```
...
自用恢复
```
mongorestore --authenticationDatabase admin -u="admin" -p="123456" -d Print ./Print
```